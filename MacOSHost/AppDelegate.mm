...
- (void)_updateMouse {
    pUpdateMouse(_mouse, _left, _right);
}
...
- (void)drawRect:(NSRect)dirtyRect {
    ...
    [_gameLock lockWhenCondition:1];
    const auto size = pActiveImage->get_size();
    const auto offset = pActiveImage->get_offset();
    ...
    if (pActivePalette) {
        for (int i = 0; i < 16; i++) {
            pActivePalette->colors[i].get(&palette[i].rgb[0], &palette[i].rgb[1], &palette[i].rgb[2]);
            buffer[i]._ = 0;
        }
    }
